<template>
  <div>
    <div>
      ...
      <div>
        ...
        <div v-if="multiKeyMode && aggregatedStats">
          ...
          <div>
            <p>有效 Keys 数</p>
            <p>{{ aggregatedStats.activeKeys }} 个</p>
          </div>
          <div v-if="invalidKeys.length > 0">
            <p>无效 Keys 数</p>
            <p>{{ invalidKeys.length }} 个</p>
          </div>
          <div>
            <p>总请求数</p>
          ...
          <div>
            ...
            <div>
              ...
              <template v-else-if="statsData.expiresAt">
                <span v-if="isApiKeyExpired(statsData.expiresAt)"> 已过期 </span>
                <span v-else-if="isApiKeyExpiringSoon(statsData.expiresAt)">
    ...
    <div v-if="!multiKeyMode && boundAccountList.length > 0">
      ...
      <div>
        <div v-for="account in boundAccountList" :key="account.id || account.key">
          <div>
            <div>
              <span> </span>
              <div>
            ...
            <div v-if="getRateLimitDisplay(account.rateLimitStatus)">
          ...
          <div v-else-if="account.platform === 'openai'">
            <div v-if="account.codexUsage">
              <div v-for="type in ['primary', 'secondary']" :key="`${account.key}-${type}`">
                <div>
                  <span>
                ...
                <div>重置剩余 {{ formatCodexRemaining(account.codexUsage?.[type]) }}</div>
              </div>
            </div>
            <p v-else>暂无额度使用数据</p>
          </div>
