      ...
      <div>
        ...
        <div v-if="apiKey">
          ...
          <div>
            ...
            <div>
              <div>
                <div v-if="showFullKey">
                  <code>{{ apiKey.key || 'Not available' }}</code>
                </div>
                <div v-else>
                  <code>{{ apiKey.keyPreview || 'cr_****' }}</code>
                </div>
              </div>
              <div>
                <button v-if="apiKey.key" @click="showFullKey = !showFullKey">
                  <svg v-if="showFullKey" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                ...
                <button v-if="showFullKey && apiKey.key" @click="copyToClipboard(apiKey.key)">
                  <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
            ...
            <p v-if="!apiKey.key">Full API key is only shown when first created or regenerated</p>
          </div>
          ...
          <div v-if="apiKey.usage">
            ...
            <div>
              ...
              <div>
                ...
                <span>{{ formatNumber(apiKey.usage.inputTokens || 0) }}</span>
              </div>
              <div>
                <span>Output Tokens:</span>
                <span>{{ formatNumber(apiKey.usage.outputTokens || 0) }}</span>
              </div>
              <div>
                <span>Total Cost:</span>
                <span>${{ (apiKey.usage.totalCost || 0).toFixed(4) }}</span>
              </div>
            </div>
          </div>
