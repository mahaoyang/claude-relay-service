      ...
      <div>
        ...
        <div>
          <div>
            ...
            <div>
              <h3>修改过期时间</h3>
              <p>为 "{{ apiKey.name || 'API Key' }}" 设置新的过期时间</p>
            </div>
          </div>
        ...
        <div>
              ...
              <div>
                ...
                <p>
                  ...
                  <template v-else-if="apiKey.expiresAt">
                    ...
                    <span v-if="getExpiryStatus(apiKey.expiresAt)">
          ...
          <div v-if="apiKey.expirationMode === 'activation' && !apiKey.isActivated">
            <button @click="handleActivateNow">
              立即激活 (激活后
              {{ apiKey.activationDays || (apiKey.activationUnit === 'hours' ? 24 : 30) }}
              {{ apiKey.activationUnit === 'hours' ? '小时' : '天' }}过期)
            </button>
          ...
          <div>
            ...
            <div>
              ...
              <button @click="selectQuickOption('custom')">自定义</button>
            </div>
          </div>
          ...
          <div v-if="localForm.expireDuration === 'custom'">
            ...
            />
            <p>选择一个未来的日期和时间作为过期时间</p>
          </div>

          <!-- 预览新的过期时间 -->
          <div v-if="localForm.expiresAt !== apiKey.expiresAt">
            <div>
              <div>
                <p>新的过期时间</p>
                <p>
                  <template v-if="localForm.expiresAt">
                    ...
                    <span v-if="getExpiryStatus(localForm.expiresAt)">
