    ...
    <div>
      <div>
        <div>
          ...
          <button title="直接关闭（不推荐）" @click="handleDirectClose"></button>
        </div>
        ...
        <div>
          ...
          <div v-if="apiKey.description">
            <label>备注</label>
            <div>
              <span>{{ apiKey.description || '无描述' }}</span>
            </div>
          </div>
          ...
          <div>
            ...
            <p>点击眼睛图标切换显示模式，使用下方按钮复制环境变量配置</p>
          </div>
        </div>
        ...
        <div>
          <div>
            <button @click="copyKeyOnly">仅复制密钥</button>
            <button @click="copyFullConfig">复制Claude配置</button>
          </div>
          <button @click="handleClose">我已保存</button>
        </div>
      </div>
    </div>
  </Teleport>
