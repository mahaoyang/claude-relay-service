<template>
  <div class="w-full">
    ...
    <div
      v-if="!limit || limit <= 0"
      class="flex items-center justify-center rounded-md px-3 py-2 text-xs"
    >
      <div class="flex items-center gap-1.5 text-gray-600 dark:text-gray-300">
        <Icon name="Infinity" class="text-sm text-gray-500 dark:text-gray-400" />
    ...
    <div v-else-if="isCompact" class="space-y-1.5">
      ...
      <div class="relative h-1.5 overflow-hidden rounded-md bg-gray-200/85 dark:bg-gray-700/70">
        <div
          class="absolute inset-y-0 rounded-md transition-all duration-500 ease-out"
    ...
    <div
      v-else
      class="group relative h-9 w-full overflow-hidden rounded-md border transition-all duration-300 ease-out"
    ...
    >
      ...
      <!-- 内部高光边框 -->
      <div
        class="pointer-events-none absolute inset-0 rounded-md border border-white/50 opacity-40 mix-blend-overlay dark:border-white/10"
      ></div>
