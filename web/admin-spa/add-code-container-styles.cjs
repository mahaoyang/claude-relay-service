...
content = content.replace(
  ...
  function(match, p1) {
    // 在前面添加容器开始标签
    const preceding = content.substring(Math.max(0, content.indexOf(match) - 200), content.indexOf(match));
    if (!preceding.includes('bg-gray-900')) {
      return '<div class="bg-gray-900 rounded-lg p-4 mb-4 overflow-x-auto space-y-1">\n            ' + match;
    }
    return match;
  }
);
...
const manualReplacements = [
  // Windows Chocolatey 代码块
  {
    ...
    replace: '如果你安装了 Chocolatey 或 Scoop，可以使用命令行安装：</p>\n            <div class="bg-gray-900 rounded-lg p-4 mb-4 overflow-x-auto space-y-1">'
  },
  // node/npm version
  {
    find: '安装完成后，打开 PowerShell 或 CMD，输入以下命令：</p>\n          <div>',
    replace: '安装完成后，打开 PowerShell 或 CMD，输入以下命令：</p>\n          <div class="bg-gray-900 rounded-lg p-4 mb-4 overflow-x-auto space-y-1">'
  },
  // claude version
  {
    find: '安装完成后，输入以下命令检查是否安装成功：</p>\n          <div>',
    replace: '安装完成后，输入以下命令检查是否安装成功：</p>\n          <div class="bg-gray-900 rounded-lg p-4 mb-4 overflow-x-auto space-y-1">'
  }
];
