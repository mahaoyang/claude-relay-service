...
function cleanFile(filePath) {
  ...
  // 3. 删除 Icon 组件（开闭标签）
  // 匹配: <Icon ...>...</Icon>
  content = content.replace(/<Icon\s+[^>]*>.*?<\/Icon>/gs, (match) => {
    changes++
    return ''
  })
  ...
  // 4. 删除 FontAwesome 图标
  // 匹配: <i class="fa-..." />
  content = content.replace(/<i\s+[^>]*class="[^"]*\bfa[s|b|r]?\s+fa-[^"]*"[^>]*\/>/g, (match) => {
    changes++
    return ''
  })
  ...
  content = content.replace(/<component\s+:is="[A-Z][a-zA-Z0-9]*Icon[^"]*"[^>]*\/>/g, (match) => {
    changes++
    return ''
  })
  ...
  content = content.replace(/<div[^>]*>\s*<(?:Icon|i|component)[^>]*\/>\s*<\/div>/g, (match) => {
    changes++
    return ''
  })
